...
class ThunderShellFlaskAPI(Flask):
    ...
    def set_user(self):
        ...
        self.redis.set_key("{}:active_users".format(str(self.session["username"])), self.session["uid"])
        Log.log_event("User Login", "%s" % str(self.session["username"]))

    def post_login(self):
        try:
            if "{}:active_users".format(str(request.form["username"].strip())) in self.redis.scan_data("*:active_users"):
                return False
    ...
    def logout(self):
        self.redis.delete_entry("{}:active_users".format(str(self.session["username"])))
        self.active_users.remove("{}".format(self.session["username"]))
